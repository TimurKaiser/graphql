async function processLogin(form) {
  ...
  const jwtRequestUrl = "https://zone01normandie.org/api/auth/signin";
  const encodedAuth = btoa(`${username}:${password}`);

  try {
    const response = await fetch(jwtRequestUrl, {
      method: "POST",
      headers: {
...
async function getData(form, requestedData) {
  const jwt = await processLogin(form);
  if (!jwt) {
    document.getElementById("error").innerText =
      "Unable to log in. Check your credentials.";
    return null;
  }
  return graphqlRequest(jwt, requestedData);
}
...
async function doStuff(form) {
  ...
  document.getElementById(
    "login"
  ).innerHTML = `<a href="${window.location.href}">Logout</a>`;
